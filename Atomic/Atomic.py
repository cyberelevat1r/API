    ...
    for attempt in range(max_retries):
        try:
            async with session.get(url) as response:
                ...
                return float(data['balance'])  # Adjust this based on the actual response structure
...
async def generate_and_check_address():
    ...
    try:
        ...
        async with print_lock:
            line_to_print = (
    f"\rAPI: Atomic Wallet | "  # Replace "Atomic Wallet" with the actual name of your API
