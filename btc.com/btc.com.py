        ...
        try:
            async with session.get(url) as response:
                ...
                if data.get('err_no') != 0:
                    raise ValueError(f"API Error: {data.get('err_msg')}")
                return float(data['data']['balance']) / 1e8  # Assuming balance is in satoshis
...
async def generate_and_check_address():
    ...
    try:
        ...
        async with print_lock:
            line_to_print = (
    f"\rAPI: btc.com | "  # Replace "btc.com" with the actual name of your API
